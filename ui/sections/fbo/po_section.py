...
class PoSection(BaseSection):
    ...
    def setup_content(self):
        ...
        header.setSectionResizeMode(2, QHeaderView.ResizeToContents)  # 발주번호
        header.setSectionResizeMode(3, QHeaderView.Stretch)           # 상품명
        header.setSectionResizeMode(4, QHeaderView.ResizeToContents)  # 수량
        header.setSectionResizeMode(5, QHeaderView.ResizeToContents)  # 발주일
    ...
    def _load_dummy_data(self):
        """테스트 목적의 더미 데이터 로드"""
        dummy_data = [
            {"seller": "판매자A", "po_number": "PO-2023-001", "product": "면 원단 30수", "quantity": 200, "date": "2023-05-15", "status": "대기중"},
            {"seller": "판매자B", "po_number": "PO-2023-002", "product": "실크 혼방 원단", "quantity": 100, "date": "2023-05-16", "status": "대기중"},
            {"seller": "판매자C", "po_number": "PO-2023-003", "product": "울 개버딘", "quantity": 50, "date": "2023-05-17", "status": "전송완료"},
            {"seller": "판매자A", "po_number": "PO-2023-004", "product": "폴리에스터 트윌", "quantity": 150, "date": "2023-05-18", "status": "전송실패"}
        ]
        ...
        for row, item in enumerate(dummy_data):
            ...
            self.table.setItem(row, 2, QTableWidgetItem(item["po_number"]))
    ...
    def _on_refresh_clicked(self):
        """새로고침 버튼 클릭 이벤트"""
        self.log("발주 확인 요청 데이터를 새로고침합니다.", LogType.INFO.value)
    ...
    def _on_send_clicked(self):
        ...
        self.log(f"{len(selected_rows)}건의 발주 확인 요청 메시지를 전송합니다.", LogType.INFO.value)
        # TODO: 실제로 메시지 전송 구현
    ...
    def on_section_activated(self):
        ...
        # '새로고침' 버튼을 클릭하여 발주 확인 요청 데이터를 가져오세요.
